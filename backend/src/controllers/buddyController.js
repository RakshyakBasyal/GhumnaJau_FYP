...
exports.sendBuddyRequest = async (req, res) => {
  try {
    ...
    const alreadyConnected = await isConnected(requester, userId);
    if (alreadyConnected) {
      return res.status(400).json({ msg: "You are already connected as travel buddies" });
    }

    let request = await BuddyRequest.findOne({
      requester,
      recipient: userId,
    });
    ...
    if (inversePending) {
      return res.status(400).json({ msg: "This user already requested you. Accept from your dashboard." });
    }
    ...
    if (io) {
      io.to(`user:${String(userId)}`).emit("buddy:request:new", {
...
exports.respondBuddyRequest = async (req, res) => {
  try {
    ...
    if (io) {
      ...
      io.to(`user:${String(request.recipient)}`).emit("buddy:request:updated", {
        requestId: String(request._id),
        status: request.status,
        byUserId: String(me),
      });
...
exports.getBuddyStatus = async (req, res) => {
  try {
    ...
    const received = await BuddyRequest.findOne({
      requester: other,
      recipient: me,
      status: "pending",
    }).select("_id");
...
exports.getConversation = async (req, res) => {
  try {
    ...
    const connected = await isConnected(me, other);
    if (!connected) {
      return res.status(403).json({ msg: "You can only chat with accepted travel buddies" });
    }

    const conversationKey = toKey(me, other);
    const messages = await BuddyMessage.find({ conversationKey })
...
exports.sendMessage = async (req, res) => {
  try {
    ...
    const message = await BuddyMessage.create({
      conversationKey: toKey(me, userId),
