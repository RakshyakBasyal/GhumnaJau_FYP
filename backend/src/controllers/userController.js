...
//         await post.save();
//         if (io) {
//           io.emit("commentDeleted", { postId, commentCount: post.commentCount });
//         }
//       })
//     );
//   }
...
//   // 7) Delete trips created by this user
//   await Trip.deleteMany({ user: userId });

//   // 8) Handle trip rooms:
//   //    - Rooms created by this user → delete entirely
//   //    - Rooms where user is a member/pending/invited → remove from those arrays
...
//         invitedBuddies: userId,
//       },
//     }
//   );
//   // Remove messages sent by this user in trip rooms
//   await TripRoom.updateMany(
...
//       // intentStatus — always save as-is when provided
//       ...(intentStatus !== undefined ? { intentStatus: String(intentStatus).trim() } : {}),
//       ...(travelDateStart !== undefined
//         ? { travelDateStart: travelDateStart ? new Date(travelDateStart) : null }
//         : {}),
//       ...(travelDateEnd !== undefined
//         ? { travelDateEnd: travelDateEnd ? new Date(travelDateEnd) : null }
//         : {}),
//     };
...
//         try { parsedStats = JSON.parse(travelStats); } catch (_) {}
...
//   try {
//     // Select ALL fields needed by UserProfile.jsx so bio, intentStatus, etc. are always correct
...
//       "preferredDestinations city languages bio travelStats gender age " +
//       "travelDateStart travelDateEnd intentStatus"
//     );
...
//     if (io) {
//       io.emit("userDeleted", { userId: req.user.id });
//     }
...
//     await Post.deleteMany({ author: userId }); // Cascade post deletion
