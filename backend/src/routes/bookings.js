...
router.post('/', auth, async (req, res) => {
  try {
    const { type, hotelId, flightId, ...rest } = req.body;
    ...
    if (type === 'hotel') {
      ...
      if (flight.availableSeats < (rest.passengers?.length || 0)) {
        return res.status(400).json({ message: 'Not enough seats' });
      }

      flight.availableSeats -= rest.passengers.length;
      await flight.save();
    }
...
router.patch('/:id/status', auth, admin, async (req, res) => {
  try {
    ...
    if (booking.status !== 'pending') {
      return res.status(400).json({ message: 'Only pending can be updated' });
    }
...
// ── USER ARCHIVE / UNARCHIVE ── (same as before)
router.patch('/my/:id/archive', auth, async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.id);
    if (!booking) return res.status(404).json({ message: 'Not found' });
...
// ── ADMIN ARCHIVE / BULK / SINGLE ── (unchanged, works for both types)
router.patch('/clear-completed', auth, admin, async (req, res) => {
  try {
    const result = await Booking.updateMany(
      { status: { $in: ['confirmed', 'cancelled'] }, isArchived: false },
