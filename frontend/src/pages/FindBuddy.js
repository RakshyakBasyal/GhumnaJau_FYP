...
const avatarUrl = (v) => getImageUrl(v);
...
export default function FindBuddy() {
  ...
  var [tripSearch, setTripSearch]   = useState({ destination: '', startDate: today, endDate: today });
    ...
    <div className="min-h-screen bg-[#f8fafc]">
              ...
              <form onSubmit={handleSearch}>
                <div className="flex flex-col md:flex-row gap-3">
                          ...
                          return (
                            <div key={dest._id} onClick={function () { handleSelectDestination(dest); }}
                              ...
                              <div className="w-8 h-8 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100">
                                {dest.images && dest.images[0] ? <img src={getImageUrl(dest.images[0])} alt={dest.name} className="w-full h-full object-cover" />
                  ...
                  <div className="relative">
                    ...
                    <input type="date" value={tripSearch.startDate} min={today} onChange={function (e) { setTripSearch(function (p) { return Object.assign({}, p, { startDate: e.target.value }); }); }}
                  ...
                  <div className="relative">
                    <Calendar className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-white/60 pointer-events-none" />
                    <input type="date" value={tripSearch.endDate} min={tripSearch.startDate || today} onChange={function (e) { setTripSearch(function (p) { return Object.assign({}, p, { endDate: e.target.value }); }); }}
                      className="w-full pl-10 pr-4 py-3 bg-white/10 border border-white/10 rounded-xl text-sm text-white focus:bg-white/20 focus:outline-none transition-all" style={{ colorScheme: 'dark' }} />
                  </div>
                  ...
                  {hasSearched && (
                    <button type="button" onClick={function () { setSearchTerm(''); setTripSearch({ destination: '', startDate: today, endDate: today }); setHasSearched(false); setSoloTravelers([]); fetchData(); }}
      ...
      {showCreateModal && (
        <CreateTripModal
          ...
          onSuccess={function () { setTab('groups'); fetchData(); }}
