      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-0.5 text-xs text-gray-400">
          ...
          {item.type !== 'destination' && item.isDone && item.actualCost != null && (
            <button onClick={() => onEditCost(item)} className={`font-semibold ${item.actualCost > item.estimatedCost ? 'text-red-500' : item.actualCost < item.estimatedCost ? 'text-green-600' : 'text-blue-600'}`}>
              Actual: {fmtNPR(item.actualCost)} ✎
            </button>
          )}
          {item.type !== 'destination' && item.isDone && item.actualCost == null && item.estimatedCost > 0 && (
            <button onClick={() => onEditCost(item)} className="text-blue-500 hover:text-blue-700">+ add actual cost</button>
          )}
        </div>
      </div>
...
// ── Plan Card — the core new concept ─────────────────────────────────────────
// A "plan" is a named step like "Take a flight to Pokhara" or "Check in to hotel"
// Under each plan you can attach flight, hotel, restaurant, activity, or expense items.
...
const PlanCard = ({ plan, items, onDeletePlan, onDeleteItem, onMarkDone, onUndone, onEditCost, onAddItemToPlan, destinationIds }) => {
  ...
  const saveTitle = async () => {
    ...
    plan.title = titleVal; // optimistic
    setEditingTitle(false);
  };

  return (
    <>
      {modal === 'flight'         && <AddFlightModal        onClose={() => setModal(null)} onAdd={handleAdd} destinationIds={destinationIds} plannedDate={plan.plannedDate} />}
      {modal === 'hotel'          && <AddHotelModal         onClose={() => setModal(null)} onAdd={handleAdd} destinationIds={destinationIds} plannedDate={plan.plannedDate} />}
      {modal === 'restaurant'     && <AddCustomModal type="restaurant" onClose={() => setModal(null)} onAdd={handleAdd} plannedDate={plan.plannedDate} />}
      {modal === 'activity'       && <AddCustomModal type="activity"   onClose={() => setModal(null)} onAdd={handleAdd} plannedDate={plan.plannedDate} />}
      {modal === 'custom_expense' && <AddCustomExpenseModal onClose={() => setModal(null)} onAdd={handleAdd} plannedDate={plan.plannedDate} />}
            ...
            {editingTitle ? (
              <div className="flex items-center gap-2">
                <input autoFocus value={titleVal} onChange={e => setTitleVal(e.target.value)}
                  ...
                  className="flex-1 text-sm font-medium text-gray-900 border border-blue-400 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-600 bg-white" />
                <button onClick={saveTitle} disabled={savingTitle} className="p-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                  {savingTitle ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}
                </button>
                <button onClick={() => { setEditingTitle(false); setTitleVal(plan.title); }} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-lg transition"><X className="h-3.5 w-3.5" /></button>
              </div>
            ) : (
...
// ── Day Section ───────────────────────────────────────────────────────────────
const DaySection = ({ day, dayNum, plans, items, onAddPlan, onDeletePlan, onAddItemToPlan, onDeleteItem, onMarkDone, onUndone, onEditCost, destinationIds, itinId, token, isToday, isTripActive }) => {
  const [open, setOpen]       = useState(true);
  const [showAddPlan, setShowAddPlan] = useState(false);
  const dateStr               = day.toISOString().slice(0, 10);

  const dayPlans = plans.filter(p => p.plannedDate && isSameDay(p.plannedDate, day));
  const dayItems = items.filter(i => i.plannedDate && isSameDay(i.plannedDate, day) && !i.planId); // unplanned items for this day
  ...
  const handleAddPlan = async (data) => {
    await onAddPlan({ ...data, plannedDate: dateStr });
  };

  return (
    <>
      {showAddPlan && <AddPlanModal onClose={() => setShowAddPlan(false)} onAdd={handleAddPlan} plannedDate={dateStr} />}

      <div className={`bg-white rounded-2xl shadow-sm overflow-hidden ${isToday && isTripActive ? 'ring-2 ring-blue-500' : ''}`}>
        {/* Day header */}
        <button onClick={() => setOpen(o => !o)}
          className={`w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition text-left ${isToday && isTripActive ? 'bg-blue-50' : ''}`}>
          <div className="flex items-center gap-3">
            <div className={`text-sm font-bold px-4 py-2 rounded-xl min-w-[5rem] text-center ${isToday && isTripActive ? 'bg-blue-600 text-white' : 'bg-blue-600 text-white'}`}>
              Day {dayNum}
            </div>
            <div>
              <div className="flex items-center gap-2">
        ...
        {open && (
          <div className="border-t border-gray-100">
              ...
              <div className="p-4 space-y-3">
                ...
                {dayItems.length > 0 && (
                  <div className="border border-gray-200 rounded-2xl px-4 py-1">
                    {dayItems.map(item => (
                      <ItemRow key={item._id} item={item} onDelete={onDeleteItem} onMarkDone={onMarkDone} onUndone={onUndone} onEditCost={onEditCost} />
                    ))}
                  </div>
                )}
              </div>
            ...
            {dayPlans.length === 0 && dayItems.length === 0 && (
              <p className="text-center text-gray-400 text-sm py-6">No plans yet for this day.</p>
            )}
            ...
            <div className="px-4 pb-4">
              <button onClick={() => setShowNotes(o => !o)}
                className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-600 font-medium transition">
...
// ── Budget Card ───────────────────────────────────────────────────────────────
const BudgetCard = ({ items, budget, onSetBudget }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={onSetBudget} className="text-xs font-semibold text-blue-600 hover:text-blue-700 transition px-2 py-1 rounded-lg hover:bg-blue-50">{budget != null ? 'Edit' : '+ Set Budget'}</button>
      </div>
        ...
        <div className="mb-4 pb-4 border-b border-gray-100">
          ...
          {hasActual && (
            <div className="mt-2 flex justify-between text-xs">
              ...
              <span className={`font-semibold ${grandAct > budget ? 'text-red-500' : 'text-green-600'}`}>{fmtNPR(grandAct)}</span>
            </div>
          )}
        </div>
      )}

      {CATS.length === 0 && <p className="text-gray-400 text-sm text-center py-3">Add items to see cost breakdown</p>}
      {CATS.length > 0 && (
        <div className="space-y-3">
          {CATS.map(c => (
            <div key={c.label} className="flex items-center justify-between text-sm">
              <span className="flex items-center gap-2 text-gray-500 flex-1 min-w-0"><c.Icon className="h-3.5 w-3.5 flex-shrink-0" /><span className="truncate">{c.label}</span></span>
              <div className="flex items-center gap-4 flex-shrink-0 ml-2">
                <span className={`font-semibold ${hasActual ? 'text-gray-400 text-xs' : 'text-gray-900'}`}>{fmtNPR(c.est)}</span>
                {hasActual && <span className={`font-semibold w-28 text-right ${c.act != null ? (c.act > c.est ? 'text-red-500' : 'text-green-600') : 'text-gray-300'}`}>{c.act != null ? fmtNPR(c.act) : '—'}</span>}
              </div>
            </div>
          ))}
          <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <span className={`font-bold ${hasActual ? 'text-gray-400 text-xs' : 'text-blue-600 text-base'}`}>{fmtNPR(grandEst)}</span>
              {hasActual && <span className={`font-bold text-base ${grandAct > grandEst ? 'text-red-500' : 'text-green-600'}`}>{fmtNPR(grandAct)}</span>}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            ...
            budget != null ? { label: 'Your Budget', value: fmtNPR(budget), unit: 'total' } : { label: 'Estimated', value: fmtNPR(totalEst), unit: 'planned' },
            hasActual ? { label: 'Actually Spent', value: fmtNPR(totalAct), unit: actDiff > 0 ? `${fmtNPR(actDiff)} over` : `${fmtNPR(Math.abs(actDiff))} under` } : null,
...
const ItineraryDetail = () => {
  ...
  const destinationIds = useMemo(() => {
    const fromItems = items
      .filter(i => i.type === 'destination')
      .map(i => String(i.referenceId))
      .filter(Boolean);
    const itineraryDestinationId = itin?.destinationId ? String(itin.destinationId) : null;
    // Fallback to itinerary-level destination when no destination item exists.
  ...
  // Unscheduled plans (no date, or no days set)
  ...
  if (error || !itin) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center"><p className="text-red-500 font-medium mb-4">{error || 'Trip not found'}</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {deletePlan && <ConfirmDelete label={deletePlan.title} onClose={() => setDeletePlan(null)} onConfirm={handleDeletePlan} />}
      ...
      {!hasDays && unschedModal === 'flight'         && <AddFlightModal        onClose={() => setUnschedModal(null)} onAdd={handleAddItem} destinationIds={destinationIds} />}
      {!hasDays && unschedModal === 'hotel'          && <AddHotelModal         onClose={() => setUnschedModal(null)} onAdd={handleAddItem} destinationIds={destinationIds} />}
      {!hasDays && unschedModal === 'restaurant'     && <AddCustomModal type="restaurant" onClose={() => setUnschedModal(null)} onAdd={handleAddItem} />}
      {!hasDays && unschedModal === 'activity'       && <AddCustomModal type="activity"   onClose={() => setUnschedModal(null)} onAdd={handleAddItem} />}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {!hasDays && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-3 text-sm text-amber-700">
                ...
                <span>Add start & end dates to enable day-by-day planning. <button onClick={() => setEditTrip(true)} className="underline font-semibold">Edit trip →</button></span>
              </div>
            )}
              ...
              <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                {plans.length > 0 ? (
                  ...
                  <div className="text-center py-14">
                    ...
                    <p className="text-gray-400 text-sm">No plans yet — add your first plan below</p>
                  </div>
                )}
          ...
          <div className="space-y-5">
            ...
            {/* Budget */}
            <BudgetCard items={items} budget={itin.budget ?? null} onSetBudget={() => setShowBudget(true)} />
...
// Small helper for no-dates case
