...
//   // ✅ COST FORMATTER (range > avg > varies)
//   const formatCost = (d) => {
//     const min = Number(d?.averageCostMin);
//     const max = Number(d?.averageCostMax);
//     const avg = Number(d?.averageCost);

//     const hasMinMax = Number.isFinite(min) && Number.isFinite(max) && min > 0 && max > 0;
//     if (hasMinMax) return `$${Math.round(min)} - $${Math.round(max)}`;

//     const hasAvg = Number.isFinite(avg) && avg > 0;
//     if (hasAvg) return `$${Math.round(avg)}`;
...
//   // show first 3 images as preview (including the first)
