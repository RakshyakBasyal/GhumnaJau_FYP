...
// 1. Removed 'rating' from formData state
// 2. Removed the rating <select> from the form
// 3. Rating in the card list is now shown as "User Rating" (read-only, computed from reviews)
// 4. Rating is no longer sent in create/update payloads
//
// The rating field on the Destination model is now automatically updated
// by postRoutes.js → updateAverageRating() whenever a user submits a review post.
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {destinations.length === 0 ? (
            ...
            destinations.map((destination) => (
              <div key={destination._id} className="bg-white rounded-xl shadow-md overflow-hidden">
                {destination.images?.[0] ? (
                  <img src={`${BASE_URL}${destination.images[0]}`} alt={destination.name} className="w-full h-48 object-cover" />
                ) : (
                  <div className="w-full h-48 bg-gray-200 flex items-center justify-center">
                    <span className="text-gray-500">No image</span>
                  </div>
