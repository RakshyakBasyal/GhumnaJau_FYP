    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="relative h-[320px] md:h-[400px] overflow-hidden">
        {displayAvatar
          ? <img src={displayAvatar} className="w-full h-full object-cover" alt={user.fullName} />
          : <div className="w-full h-full bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 flex items-center justify-center">
              <span className="text-white text-[140px] font-black opacity-10 select-none">{user.fullName?.charAt(0)}</span>
            </div>}
        ...
        <button onClick={() => navigate(-1)}
          className="absolute top-5 left-5 bg-black/30 backdrop-blur-sm text-white p-2 rounded-xl border border-white/10 hover:bg-black/50 transition">
          <ChevronLeft size={18} />
        </button>
        ...
        {displayAvatar && (
          <button onClick={() => setShowPhotoView(true)}
            className="absolute top-5 left-14 bg-black/30 backdrop-blur-sm text-white text-xs font-medium px-3 py-2 rounded-xl flex items-center gap-1.5 border border-white/10 hover:bg-black/50 transition">
            <Camera size={13} /> View Photo
          </button>
        )}
        ...
        {/* Compatibility badge */}
        {user.compatibilityScore > 0 && (
          <div className={`absolute top-5 right-5 ${scoreColor} text-white text-xs font-bold px-3 py-1.5 rounded-xl shadow-lg`}>
            {user.compatibilityScore}% match
          </div>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="flex items-end justify-between gap-4 mb-3">
              <div className="flex-1 min-w-0">
                <h1 className="text-3xl md:text-4xl font-bold text-white leading-tight truncate">{user.fullName}</h1>
                <div className="flex flex-wrap items-center gap-2 mt-2">
                  {user.city && (
                    <span className="text-white/75 text-sm flex items-center gap-1">
                      <MapPin size={13} className="text-blue-400 flex-shrink-0" /> {user.city}
                    </span>
                  )}
                  {user.intentStatus && (
                    <span className="px-2.5 py-0.5 bg-blue-500/70 backdrop-blur-sm text-white text-xs font-medium rounded-full border border-white/10">
                      {user.intentStatus}
                    </span>
                  )}
                  {user.travelStyle && (
                    <span className="px-2.5 py-0.5 bg-white/15 backdrop-blur-sm text-white text-xs font-medium rounded-full border border-white/10">
                      {user.travelStyle}
                    </span>
                  )}
                  {(user.gender || user.age) && (
                    <span className="px-2.5 py-0.5 bg-white/15 backdrop-blur-sm text-white/80 text-xs rounded-full border border-white/10">
                      {user.gender}{user.age ? ` · ${user.age}yrs` : ''}
                    </span>
                  )}
                </div>
              ...
              {!isSelf && (
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {isConnected ? (
                    <button onClick={() => navigate('/community/messages')}
                      className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition flex items-center gap-1.5 shadow-lg">
                      <MessageSquare size={14} /> Message
                    </button>
                  ) : (
                    <button onClick={handleConnect} disabled={connectBusy}
                      className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition flex items-center gap-1.5 disabled:opacity-60 shadow-lg">
                      {connectBusy ? <Loader2 size={14} className="animate-spin" /> : <UserPlus size={14} />}
                      Connect
                    </button>
                  )}
                </div>
              ...
              {isSelf && (
                <button onClick={() => navigate('/profile')}
                  className="px-4 py-2 bg-white/15 backdrop-blur-sm text-white border border-white/20 rounded-xl text-sm font-medium hover:bg-white/25 transition">
                  Edit Profile
                </button>
              )}
            </div>
            ...
            {/* Stats strip */}
            <div className="flex items-center gap-0.5 bg-black/35 backdrop-blur-sm rounded-2xl px-1 py-0.5 w-fit border border-white/10">
              {[
                { val: postCount,                       label: 'Posts',     onClick: null },
                { val: stats.followersCount,             label: 'Followers', onClick: () => setFollowModal('followers') },
                { val: stats.followingCount,             label: 'Following', onClick: () => setFollowModal('following') },
                ...
                <div key={s.label} className="flex items-center">
                  {i > 0 && <div className="w-px h-7 bg-white/15 mx-0.5" />}
                  <button onClick={s.onClick || undefined} disabled={!s.onClick}
                    className={`flex flex-col items-center px-4 py-2 rounded-xl transition ${s.onClick ? 'hover:bg-white/10 cursor-pointer' : 'cursor-default'}`}>
                    <span className="text-lg font-bold text-white leading-none">{s.val}</span>
                    <span className="text-[9px] text-white/50 font-medium uppercase tracking-wider mt-0.5">{s.label}</span>
                  </button>
                </div>
              ))}
            </div>
