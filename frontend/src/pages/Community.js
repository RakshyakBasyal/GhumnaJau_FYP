...
import { useLocation, useNavigate } from 'react-router-dom';
import { Bell, Heart, LayoutDashboard, MessageCircle, MessageSquare, Users, X } from 'lucide-react';
import { io } from 'socket.io-client';
import { getBuddyConnections, getBuddyMessages, getBuddyRequests, getMe, sendBuddyMessage } from '../services/api';
import { useToast } from '../context/ToastContext';
import Feed from './Feed';
...
const Community = () => {
  ...
  useEffect(() => {
    ...
    return () => socket.disconnect();
  }, [myId, activeChatBuddy, loadBuddyRequestsAsNotifications, pushNotif]);
                      ...
                      <>
                        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold text-gray-900">{activeChatBuddy.fullName}</p>
                              <p className="text-xs text-emerald-500 font-medium">Connected buddy</p>
                            </div>
                          </div>
                        </div>
