...
import { io } from 'socket.io-client';
import { getConnections, getBuddyMessages, getMe, sendBuddyMessage, createTripFromChat } from '../services/api';
import { useToast } from '../context/ToastContext';
import Feed from './Feed';
import FindBuddy from './FindBuddy';
...
const Community = () => {
  ...
  const handlePlanTrip = async (e) => {
    e.preventDefault();
    if (!planForm.destination || !planForm.startDate) { showToast('Destination and start date are required', 'error'); return; }
    setPlanLoading(true);
    try {
      await createTripFromChat({ partnerId: activeChatBuddy._id, destination: planForm.destination, startDate: planForm.startDate, endDate: planForm.endDate || planForm.startDate, budget: planForm.budget || '' });
      showToast(`Trip to ${planForm.destination} created!`, 'success');
      setShowPlanModal(false); setPlanForm({ destination: '', startDate: '', endDate: '', budget: '' });
    } catch (err) { showToast(err?.response?.data?.msg || 'Failed to create trip', 'error'); }
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
              <div>
                <h3 className="font-bold text-gray-900 text-lg">Plan a Trip Together</h3>
                <p className="text-xs text-gray-400 mt-0.5">with {activeChatBuddy.fullName} — both of you become co-owners</p>
              </div>
              <button onClick={() => setShowPlanModal(false)} className="p-2 hover:bg-gray-100 rounded-xl"><X size={17} className="text-gray-500" /></button>
            </div>
            <form onSubmit={handlePlanTrip} className="p-6 space-y-4">
              <div>
                ...
                <div className="relative">
                  <MapPin size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                  <input required type="text" placeholder="e.g. Pokhara, Nepal" value={planForm.destination}
                    onChange={e => setPlanForm(p => ({ ...p, destination: e.target.value }))}
                    className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-blue-400 outline-none" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Start *</label>
                  <input required type="date" value={planForm.startDate} onChange={e => setPlanForm(p => ({ ...p, startDate: e.target.value }))}
                    className="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-blue-400 outline-none" />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5">End</label>
                  <input type="date" min={planForm.startDate} value={planForm.endDate} onChange={e => setPlanForm(p => ({ ...p, endDate: e.target.value }))}
                    className="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-blue-400 outline-none" />
                </div>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Budget</label>
                <select value={planForm.budget} onChange={e => setPlanForm(p => ({ ...p, budget: e.target.value }))}
                  className="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-blue-400 outline-none">
                  <option value="">Optional</option>
                  <option>Budget</option><option>Mid-range</option><option>Luxury</option><option>Flexible</option>
                </select>
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowPlanModal(false)} className="flex-1 py-2.5 text-sm font-bold text-gray-500 hover:bg-gray-50 rounded-xl">Cancel</button>
                <button type="submit" disabled={planLoading} className="flex-1 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 disabled:opacity-60 flex items-center justify-center gap-2">
                  {planLoading ? <><Loader2 size={14} className="animate-spin" /> Creating...</> : <><Sparkles size={14} /> Create Trip & Group</>}
                </button>
              </div>
            </form>
          </div>
