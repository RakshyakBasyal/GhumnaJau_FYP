...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      console.log('Admin Dashboard Socket connected');
      // Fetch latest bookings on connect to sync
      fetchBookings();
    });
    ...
    socket.on('bookingUpdated', (updatedBooking) => {
      console.log('Booking updated via socket:', updatedBooking._id, updatedBooking.status);
      setBookings((prev) => {
      ...
      showToast(`Booking updated to ${updatedBooking.status}`, 'info');
      // Force refresh full list for consistency
      fetchBookings();
    });

    // Optional: specific cancelled event (if you add emit('bookingCancelled') in backend)
    socket.on('bookingCancelled', (cancelledBooking) => {
      console.log('Booking cancelled via socket:', cancelledBooking._id);
      setBookings((prev) =>
        prev.map((b) =>
          b._id === cancelledBooking._id ? { ...b, ...cancelledBooking } : b
        )
      );
