...
// // import {
// //   getMe, getDiscoverUsers,
// //   getBuddyRequests, getBuddyConnections, getDestinations, connectUser
// // } from '../services/api';
...
// //       <button
// //         onClick={onExplore}
// //         className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-lg transition shadow"
// //       >
// //         Explore Posts
// //       </button>
// //     </div>
...
// //       setConnectMap(function (prev) { return Object.assign({}, prev, { [userId]: 'connected' }); });
...
// //           <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
// //             <div>
// //               <h2 className="text-xl font-bold text-gray-900 tracking-tight">Community Feed</h2>
...
// //                 className="flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg shadow transition-all">
// //                 <PenSquare size={16} /> Share Moment
// //               </button>
// //               <button onClick={function () { setHasNew(false); fetchPosts(true); }}
// //                 className="p-2 rounded-lg bg-gray-50 text-gray-500 hover:bg-gray-100 transition-all border border-gray-100">
...
// //               </h1>
// //               <div className="flex items-center gap-2">
// //                 <Link to="/profile" className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-700 text-sm font-semibold transition">
// //                   {userAvatar
...
// //                   <button onClick={function () { setHasNew(false); fetchPosts(true); }} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 transition">
// //                     <RefreshCw size={17} />
// //                   </button>
// //                   {hasNew && <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-red-500 border-2 border-white" />}
// //                 </div>
// //                 <button onClick={function () { setEditingPost(null); setShowCreate(true); }} className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-lg shadow transition">
...
// //                         className={'text-xs px-3 py-1.5 rounded-lg border font-semibold transition ' + (category === cat.value ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50')}>
// //                         {cat.label}
// //                       </button>
// //                     );
// //                   })}
...
// //                         <span className="text-[8px] font-bold text-blue-600 uppercase">Hot</span>
// //                       </div>
// //                     </Link>
// //                   );
// //                 })}
...
// //                   </div>}
// //               <div>
// //                 <p className="text-sm font-bold">{storyCurrentGrouped.author && storyCurrentGrouped.author.fullName}</p>
...
//     // Remove all posts from a deleted user in real time
//     socket.on('userDeleted', function (data) {
//       var deletedUserId = data.userId;
//       setPosts(function (prev) {
//         return prev.filter(function (p) {
//           return String(p.author && (p.author._id || p.author)) !== String(deletedUserId);
//         });
//       });
//       setStories(function (prev) {
//         return prev.filter(function (s) {
//           return String(s.author && (s.author._id || s.author)) !== String(deletedUserId);
//         });
//       });
//     });
...
import PostCard from '../components/feed/PostCard';
import CreatePostModal from '../components/feed/CreatePostModal';
import { getExploreFeed, getFollowingFeed, followUser, unfollowUser } from '../services/feedApi';
import {
  getMe, getDiscoverUsers, getDestinations,
} from '../services/api';
import { useToast } from '../context/ToastContext';
  ...
  async function handleConnect(userId, currentStatus) {
    try {
      if (currentStatus === 'following') {
        ...
        setConnectMap(function (prev) { var m = Object.assign({}, prev); delete m[userId]; return m; });
        showToast('Unfollowed', 'info');
      } else {
        await followUser(userId);
        setConnectMap(function (prev) { return Object.assign({}, prev, { [userId]: 'following' }); });
                ...
                {suggested.map(function (person) {
                  ...
                  return (
                    <div key={person._id} className="flex items-center justify-between group">
                      ...
                      <button onClick={function () { handleConnect(person._id, status); }}
                        className={'text-xs font-bold transition ' + (status === 'following' ? 'text-gray-400 hover:text-red-500' : 'text-blue-600 hover:text-blue-800')}>
