...
const PhotoGrid = ({ images, onPhotoClick }) => {
  ...
  if (count === 2) return <div className="grid grid-cols-2 gap-0.5 h-72">{images.map((img, i) => renderImage(img, 'h-full', i))}</div>;
  if (count === 3) return (
    <div className="grid grid-cols-2 gap-0.5 h-80">
      {renderImage(images[0], 'h-full row-span-2', 0)}
      {renderImage(images[1], 'h-full', 1)}
  ...
  if (count === 4) return <div className="grid grid-cols-2 gap-0.5 h-80">{images.slice(0, 4).map((img, i) => renderImage(img, 'h-full', i))}</div>;
  return (
    <div className="grid grid-cols-6 gap-0.5 h-80">
      {renderImage(images[0], 'col-span-3 row-span-2 h-full', 0)}
      {renderImage(images[1], 'col-span-3 row-span-1 h-full', 1)}
...
export default function PostCard({ post, onUpdated, onDeleted }) {
  ...
  // ── FIX: actually call the delete API before notifying parent ──────────────
  ...
  const handleLikeAnswer = async (answerId) => {
    try {
      ...
      setAnswers(prev => prev.map(a => {
        if (String(a._id) !== String(answerId)) return a;
        const wasLiked = a.likes.some(l => String(l) === String(myId));
        const newLikes = wasLiked ? a.likes.filter(l => String(l) !== String(myId)) : [...a.likes, myId];
        return { ...a, likes: newLikes, likeCount: newLikes.length };
      }));
    } catch (_) {}
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      {/* ── Content — BEFORE images so caption is clearly tied to author ── */}
      {content && (
        <div className="px-4 pb-3">
          {/* Caption text — medium weight, clearly body text */}
      ...
      {viewerIdx >= 0 && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center" onClick={() => setViewerIdx(-1)}>
          ...
          <div className="relative max-w-5xl w-full flex flex-col items-center justify-center p-4" onClick={e => e.stopPropagation()}>
            <img src={BASE_URL + images[viewerIdx]} alt="" className="max-w-full max-h-[85vh] object-contain rounded-xl" />
            {images.length > 1 && (
                ...
                return (
                  <div key={c._id} className="flex items-start gap-2">
                    ...
                    <div className="flex-1 bg-gray-50 rounded-xl px-3 py-2 group">
                      ...
                      {/* Comment text — clear body weight */}
